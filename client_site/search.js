...
//     let url = 'http://localhost:3060/api/SEARCH/one?organizer=${organizer}&city=${city}&category=${category}';   
...
//             document.getElementById('records').textContent = "Load failure";
//         });
        ...
        fetch(url)
         ...
         .then(data => {
                ...
                if (data.length > 0) {
                    data.forEach(fundraiser => {
                        const newP = document.createElement("p");
                        newP.textContent = `Findraiser ID:${fundraiser.FUNDRAISER_ID},ORGANIZER:${fundraiser.ORGANIZER}, CAPTION: ${fundraiser.CAPTION}, TARGET_FUNDING: ${fundraiser.TARGET_FUNDING}, CURRENT_FUNDING: ${fundraiser.CURRENT_FUNDING}`;
                        newP.addEventListener("click", function () {
                            localStorage.setItem("ORGANIZER", fundraiser.ORGANIZER);
                            location.href = '/fundraiser';
                        });
